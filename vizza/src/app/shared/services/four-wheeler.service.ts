...
//// home page
 ...
 getPremieumList1(data) {
  ...
  getCityListin(data) {
    ...
    const url = this.configurationService.getFourwheelerInsurance() + 'royalsundaram/addressCityList';
    return this.http.post(url,json, httpOptions)
        .map(this.extractData)
        .catch(this.handleError);
  }
...
// policy
  ...
  getTowingDetails(data) {
    const json = JSON.stringify(data);
    const token = this.authService.getAccessToken();
    const httpOptions = {
      headers: new HttpHeaders({'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*'})
    };
    const url = this.configurationService.getFourwheelerInsurance() + 'royalsundaram/towingValue';
    return this.http.post(url,json, httpOptions)
        .map(this.extractData)
        .catch(this.handleError);
  }
...
// shriram //
  ...
  getPAExclusionList(data) {
    ...
    const url = this.configurationService.getFourwheelerInsurance() + 'common/PA_exclusion_list';
    return this.http.post(url,json, httpOptions)
        .map(this.extractData)
        .catch(this.handleError);
  }
